# PasswordSaver
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](https://github.com/nikhilbhatt/Android-java-PasswordSaver/blob/master/LICENSE)   [![Contributors](https://img.shields.io/badge/CoNTRIBUTORS-1-red)](https://github.com/nikhilbhatt/Android-java-PasswordSaver/graphs/contributors)  [![Built for](https://img.shields.io/badge/BUILT%20FOR-ANDORID-brightgreen)](https://www.android.com/)

Password saver is an android app used to store your username and password of any website.
For example:- Facebook password,Gmail and many more.

1. Master password of app is stored using sharedprefernces.
2. All the username and password are displayed in Recycler view using adapter calss which are stored in Sqlite database.

# Platform
      -> Android Studio 
      -> Java

# Instructions
   1. Clone or Download the repo https://github.com/nikhilbhatt/Android-java-PasswordSaver.
   2. Navigate to folder Android-java-PasswordSaver
   3. Navigate to folder Android-java-Password-saver/app/src to access developer content.

When you install the app first time in your device then it ask to create a new password which look like this.

<img src="https://github.com/nikhilbhatt/PasswordSaver/blob/master/password%20saver/Screenshot_20190405-222511.png" width="350" height="600">

# FEATURES


*1. Add username and password by clicking + button in bottom right of screen.*


<img src="https://github.com/nikhilbhatt/PasswordSaver/blob/master/password%20saver/Screenshot_20190405-222804.png" width="350" height="600"          >           <img src="https://github.com/nikhilbhatt/PasswordSaver/blob/master/password%20saver/Screenshot_20190405-222833.png" width="350" height="600" >


*2. Click on three dots to check more features of app.*


<img src="https://github.com/nikhilbhatt/PasswordSaver/blob/master/password%20saver/Screenshot_20190405-222523.png" width="350" height="600" >     <img src="https://github.com/nikhilbhatt/PasswordSaver/blob/master/password%20saver/Screenshot_20190405-222530.png" width="350" height="600">


*3. Enable the delete button to delete any of password by swiping it left or right.*


*4. Update the password of app.*


<img src="https://github.com/nikhilbhatt/PasswordSaver/blob/master/password%20saver/Screenshot_20190405-222542.png" width="350" height="600">       
